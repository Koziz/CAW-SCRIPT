-- [[ KZOYZ HUB - MANAGER MODULE ]] --

getgenv().ScriptVersion = "Manager v1.0" 
...
-- Cleanup UI Lama
pcall(function() if getgenv().KzoyzManagerUI then getgenv().KzoyzManagerUI:Destroy() end end)

-- UI Setup
local ScreenGui = Instance.new("ScreenGui"); ScreenGui.Name = "KzoyzManagerUI"; pcall(function() ScreenGui.Parent = CoreGui end); if not ScreenGui.Parent then ScreenGui.Parent = LP.PlayerGui end; ScreenGui.IgnoreGuiInset = true; ScreenGui.ResetOnSpawn = false; getgenv().KzoyzManagerUI = ScreenGui 
...
local Main = Instance.new("Frame"); Main.Parent = ScreenGui; Main.BackgroundColor3 = Theme.Bg; Main.Position = UDim2.new(0.6, 320, 0.5, -140); Main.Size = UDim2.new(0, 300, 0, 330); local MC = Instance.new("UICorner"); MC.CornerRadius = UDim.new(0, 10); MC.Parent = Main; local MS = Instance.new("UIStroke"); MS.Parent = Main; MS.Color = Theme.Purple; MS.Thickness = 1.5; MS.Transparency = 0.5
...
local Title = Instance.new("TextLabel"); Title.Parent = Header; Title.Text = "💼 Manager"; Title.TextColor3 = Theme.Text; Title.Font = Enum.Font.GothamBold; Title.TextSize = 15; Title.Size = UDim2.new(0, 150, 1, 0); Title.Position = UDim2.new(0, 15, 0, 0); Title.BackgroundTransparency = 1; Title.TextXAlignment = Enum.TextXAlignment.Left
local MinBtn = Instance.new("TextButton"); MinBtn.Parent = Header; MinBtn.Text = "X"; MinBtn.TextColor3 = Color3.fromRGB(255, 100, 100); MinBtn.Font = Enum.Font.GothamBold; MinBtn.TextSize = 16; MinBtn.Size = UDim2.new(0, 40, 0, 40); MinBtn.Position = UDim2.new(1, -40, 0, 0); MinBtn.BackgroundTransparency = 1; MinBtn.MouseButton1Click:Connect(function() ManageUIState("Normal"); ScreenGui:Destroy() end)
...
function CreateTextBox(Parent, Text, Default, Var) local Frame = Instance.new("Frame"); Frame.Parent = Parent; Frame.BackgroundColor3 = Theme.Item; Frame.Size = UDim2.new(1, 0, 0, 35); local C = Instance.new("UICorner"); C.CornerRadius = UDim.new(0, 6); C.Parent = Frame; local Label = Instance.new("TextLabel"); Label.Parent = Frame; Label.Text = Text; Label.TextColor3 = Theme.Text; Label.BackgroundTransparency = 1; Label.Size = UDim2.new(0.5, 0, 1, 0); Label.Position = UDim2.new(0, 10, 0, 0); Label.Font = Enum.Font.GothamSemibold; Label.TextSize = 12; Label.TextXAlignment = Enum.TextXAlignment.Left; local InputBox = Instance.new("TextBox"); InputBox.Parent = Frame; InputBox.BackgroundColor3 = Color3.fromRGB(30, 30, 30); InputBox.Position = UDim2.new(0.6, 0, 0.15, 0); InputBox.Size = UDim2.new(0.35, 0, 0.7, 0); InputBox.Font = Enum.Font.GothamSemibold; InputBox.TextSize = 12; InputBox.TextColor3 = Theme.Text; InputBox.Text = tostring(Default); local IC = Instance.new("UICorner"); IC.CornerRadius = UDim.new(0, 4); IC.Parent = InputBox; InputBox.FocusLost:Connect(function() local val = tonumber(InputBox.Text); if val then getgenv()[Var] = val else InputBox.Text = tostring(getgenv()[Var]) end end); return InputBox end
function CreateButton(Parent, Text, Callback) local Btn = Instance.new("TextButton"); Btn.Parent = Parent; Btn.BackgroundColor3 = Theme.Purple; Btn.Size = UDim2.new(1, 0, 0, 35); Btn.Text = Text; Btn.TextColor3 = Color3.new(1,1,1); Btn.Font = Enum.Font.GothamBold; Btn.TextSize = 12; local C = Instance.new("UICorner"); C.CornerRadius = UDim.new(0, 6); C.Parent = Btn; Btn.MouseButton1Click:Connect(Callback) end

CreateToggle(Page, "Enable Auto Collect", "AutoCollect")
local BoxX = CreateTextBox(Page, "Target Grid X", 0, "TargetPosX")
local BoxY = CreateTextBox(Page, "Target Grid Y", 0, "TargetPosY")
CreateButton(Page, "Save Set Pos (Current Location)", function()
...
CreateToggle(Page, "Auto Drop", "AutoDrop", function(state) if not state then ManageUIState("Normal") end end)
CreateSlider(Page, "Drop Amount", 1, 200, 50, "DropAmount")
